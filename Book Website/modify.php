<?php
$page_title = 'Books - Modify a Book';
$heading = "Modify a Book";
include ('header.htm');

	if ($errorcode != 1) {		
		echo '<p><form name="add" action="modify.php" method="POST">
			ISBN: <input type="text" name="ISBN" /></t>
			<input type="submit" name="MODIFYBOOK" value="Get Details" /></form>';
		...
		if (isset($_REQUEST['MODIFYBOOK'])){
			...
			if (mysql_num_rows($r) == 1) {
				...
				echo '<p><form name="sendupdate" action="update.php" method="POST">
			<table>
			...
			<tr><td colspan="2"><input type="submit" name="UPDATECHANGES" value="Update Details" text="Add Book" /></td></tr>
			</table>
			</form></p>';
			}
			else {
